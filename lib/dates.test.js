...
describe('user-facing date formatting and parsing', () => {
  it('formatDateToUser formats ISO dates according to preference', () => {
    expect(formatDateToUser('2026-12-31', 'dmy', 'en')).toBe('31/12/2026')
    expect(formatDateToUser('2026-12-31', 'mdy', 'en')).toBe('12/31/2026')
    expect(formatDateToUser('2026-12-31', 'ymd', 'en')).toBe('2026-12-31')
  })

  it('parseUserDateToIso parses formatted text back to ISO YYYY-MM-DD', () => {
    expect(parseUserDateToIso('31/12/2026', 'dmy', 'en')).toBe('2026-12-31')
    expect(parseUserDateToIso('12/31/2026', 'mdy', 'en')).toBe('2026-12-31')
    expect(parseUserDateToIso('2026-12-31', 'ymd', 'en')).toBe('2026-12-31')
    expect(parseUserDateToIso('31.12.2026', 'dmy', 'en')).toBe('2026-12-31')
    expect(parseUserDateToIso('invalid', 'dmy', 'en')).toBe(null)
    expect(parseUserDateToIso('31/02/2026', 'dmy', 'en')).toBe(null) // Feb 31 invalid
  })

  it('formatDateTimeToUser formats local ISO datetimes according to preferences', () => {
    expect(formatDateTimeToUser('2026-12-31T14:30', 'dmy', 'h24', 'en')).toBe('31/12/2026 14:30')
    expect(formatDateTimeToUser('2026-12-31T14:30', 'mdy', 'h12', 'en')).toBe('12/31/2026 02:30 PM')
    expect(formatDateTimeToUser('2026-12-31T09:05', 'ymd', 'h12', 'en')).toBe('2026-12-31 09:05 AM')
  })

  it('parseUserDateTimeToIso parses formatted datetime text back to YYYY-MM-DDTHH:mm', () => {
    expect(parseUserDateTimeToIso('31/12/2026 14:30', 'dmy', 'h24', 'en')).toBe('2026-12-31T14:30')
    expect(parseUserDateTimeToIso('12/31/2026 2:30 PM', 'mdy', 'h12', 'en')).toBe('2026-12-31T14:30')
    expect(parseUserDateTimeToIso('2026-12-31 09:05 AM', 'ymd', 'h12', 'en')).toBe('2026-12-31T09:05')
  })

  it('provides placeholders based on preferences', () => {
    expect(getDatePlaceholder('dmy', 'en')).toBe('dd/mm/yyyy')
    expect(getDatePlaceholder('mdy', 'en')).toBe('mm/dd/yyyy')
    expect(getDatePlaceholder('ymd', 'en')).toBe('yyyy-mm-dd')
    expect(getDateTimePlaceholder('dmy', 'h24', 'en')).toBe('dd/mm/yyyy hh:mm')
    expect(getDateTimePlaceholder('mdy', 'h12', 'en')).toBe('mm/dd/yyyy hh:mm AM/PM')
  })
})
