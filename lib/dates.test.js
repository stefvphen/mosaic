...
describe('formatDateValue — date-only answers per preference', () => {
  it.each(LOCALES)('forces the digit order regardless of locale (%s)', (locale) => {
    expect(formatDateValue('2026-12-31', locale, { dateFormat: 'dmy' })).toBe('31/12/2026')
    expect(formatDateValue('2026-12-31', locale, { dateFormat: 'mdy' })).toBe('12/31/2026')
    expect(formatDateValue('2026-12-31', locale, { dateFormat: 'ymd' })).toBe('2026-12-31')
  })
  it('empty/invalid input yields empty string', () => {
    expect(formatDateValue('', 'en', { dateFormat: 'dmy' })).toBe('')
    expect(formatDateValue(undefined, 'en', {})).toBe('')
  })
  it('never shifts the calendar day (interpreted as UTC)', () => {
    // A naive date must render the same day it stores, in any runtime tz.
    expect(formatDateValue('2026-01-01', 'en', { dateFormat: 'mdy' })).toBe('01/01/2026')
  })
})

describe('formatLocalInput — datetime-local wall-clock per preference', () => {
  const V = '2026-12-31T14:30'
  it('forces date order + 24h time', () => {
    expect(formatLocalInput(V, 'en', { dateFormat: 'dmy', timeFormat: 'h24' })).toBe('31/12/2026 14:30')
    expect(formatLocalInput(V, 'en', { dateFormat: 'mdy', timeFormat: 'h24' })).toBe('12/31/2026 14:30')
    expect(formatLocalInput(V, 'en', { dateFormat: 'ymd', timeFormat: 'h24' })).toBe('2026-12-31 14:30')
  })
  it('forces 12h time with AM/PM', () => {
    expect(formatLocalInput(V, 'en', { dateFormat: 'dmy', timeFormat: 'h12' })).toBe('31/12/2026 2:30 PM')
    expect(formatLocalInput('2026-12-31T00:05', 'en', { dateFormat: 'dmy', timeFormat: 'h12' })).toBe('31/12/2026 12:05 AM')
    expect(formatLocalInput('2026-12-31T12:00', 'en', { dateFormat: 'dmy', timeFormat: 'h12' })).toBe('31/12/2026 12:00 PM')
  })
  it('does not convert timezones (wall-clock stays put)', () => {
    // Same digits regardless of the machine timezone.
    expect(formatLocalInput(V, 'en', { dateFormat: 'ymd', timeFormat: 'h24' })).toBe('2026-12-31 14:30')
  })
  it('empty input yields empty string', () => {
    expect(formatLocalInput('', 'en', {})).toBe('')
  })
})

describe('dateInputPattern — placeholder hints', () => {
  it('explicit formats map to letter patterns', () => {
    expect(dateInputPattern('date', { dateFormat: 'dmy' }, 'en')).toBe('DD/MM/YYYY')
    expect(dateInputPattern('date', { dateFormat: 'mdy' }, 'en')).toBe('MM/DD/YYYY')
    expect(dateInputPattern('date', { dateFormat: 'ymd' }, 'en')).toBe('YYYY-MM-DD')
  })
  it('datetime-local appends a time pattern', () => {
    expect(dateInputPattern('datetime-local', { dateFormat: 'dmy', timeFormat: 'h24' }, 'en')).toBe('DD/MM/YYYY, HH:MM')
    expect(dateInputPattern('datetime-local', { dateFormat: 'mdy', timeFormat: 'h12' }, 'en')).toBe('MM/DD/YYYY, HH:MM AM')
  })
  it('auto never throws and returns a non-empty pattern', () => {
    expect(dateInputPattern('date', { dateFormat: 'auto' }, 'en').length).toBeGreaterThan(0)
    expect(dateInputPattern('datetime-local', {}, 'fr').length).toBeGreaterThan(0)
  })
})
