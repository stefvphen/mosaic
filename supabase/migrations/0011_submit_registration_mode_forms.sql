-- Mode-scoped forms (0010) mean a participant's answers may come from the
-- event's single/family form instead of the type's own form. Relax the
-- form-version check accordingly: the snapshotted version must be a
-- PUBLISHED version of a form belonging to THIS event. /api/register (the
-- only permitted caller) resolves which form applies per registration mode.
